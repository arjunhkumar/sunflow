...
public class BoundingIntervalHierarchy implements AccelerationStructure {
    ...
    private void buildHierarchy(IntArray tempTree, int[] indices, BuildStats stats) {
        ...
        if (objects.length == 0)
            return;
        // seed bbox
        float[] gridBox = { bounds.getMinimum().getX(), bounds.getMaximum().getX(),
                bounds.getMinimum().getY(), bounds.getMaximum().getY(),
                bounds.getMinimum().getZ(), bounds.getMaximum().getZ() };
        float[] nodeBox = { bounds.getMinimum().getX(), bounds.getMaximum().getX(),
                bounds.getMinimum().getY(), bounds.getMaximum().getY(),
                bounds.getMinimum().getZ(), bounds.getMaximum().getZ() };
