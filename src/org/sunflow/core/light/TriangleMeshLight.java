...
public class TriangleMeshLight extends TriangleMesh implements Shader, LightSource {
    ...
    public void getPhoton(double randX1, double randY1, double randX2, double randY2, Point3 p, Vector3 dir, Color power) {
        ...
        float x = w * points[index0 + 0] + u * points[index1 + 0] + v * points[index2 + 0];
        float y = w * points[index0 + 1] + u * points[index1 + 1] + v * points[index2 + 1];
        float z = w * points[index0 + 2] + u * points[index1 + 2] + v * points[index2 + 2];
        x += 0.001f * ngs[j].x;
        y += 0.001f * ngs[j].y;
        z += 0.001f * ngs[j].z;
        p = new Point3(x, y, z);
